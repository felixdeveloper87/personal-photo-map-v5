...
import { Box, Container, VStack, Heading, Text, Grid, Badge, HStack, Icon, useColorModeValue } from '@chakra-ui/react';
import { motion } from 'framer-motion';
import { HiGlobeAlt, HiChartBar, HiCamera, HiRocketLaunch } from 'react-icons/hi2';

const MotionBox = motion.create(Box);
const learningSteps = [
  { step: '01', title: 'Explore Countries', description: 'Click on any country to access comprehensive educational data and statistics', icon: HiGlobeAlt },
  { step: '02', title: 'Analyze Data', description: 'Study economic indicators, social metrics, and cultural information from trusted sources', icon: HiChartBar },
  { step: '03', title: 'Document & Learn', description: 'Upload photos and create a personal learning journey with visual context', icon: HiCamera }
];
      ...
      <Container maxW="container.xl">
        <VStack spacing={24}>
          <VStack spacing={10} textAlign="center" maxW="900px">
            <Badge colorScheme="teal" variant="solid" px={6} py={3} borderRadius="full" fontSize="lg" fontWeight="semibold" bg={badgeBg} color="white" boxShadow="0 4px 6px -1px rgba(20, 184, 166, 0.2)">
            ...
            {learningSteps.map((item, index) => (
              <MotionBox key={index} initial={{ opacity: 0, y: 40 }} whileInView={{ opacity: 1, y: 0 }} transition={{ duration: 0.8, delay: index * 0.2, ease: 'easeOut' }} viewport={{ once: true }}>
                <VStack spacing={8} textAlign="center" align="stretch">
                  <VStack spacing={6}>
                    <Box p={5} bgGradient="linear(135deg, blue.400, blue.600)" borderRadius="2xl" color="white" boxShadow="lg" boxSize="60px" display="flex" alignItems="center" justifyContent="center">
